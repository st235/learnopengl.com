...
const char* kOrangeFragmentShader = R"(
#version 330 core
out vec4 FragColor;
...
const char* kYellowFragmentShader = R"(
#version 330 core
out vec4 FragColor;
...
int main() {
    ...
    glShaderSource(orange_fragment_shader, 1, &kOrangeFragmentShader, nullptr);
    glCompileShader(orange_fragment_shader);
    if (!CheckShaderCompilation(orange_fragment_shader)) {
        std::abort();
        return 1;
    }

    unsigned int yellow_fragment_shader = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(yellow_fragment_shader, 1, &kYellowFragmentShader, nullptr);
    glCompileShader(yellow_fragment_shader);
    if (!CheckShaderCompilation(yellow_fragment_shader)) {
        std::abort();
        return 1;
    }
    ...
    if (!CheckProgramCompilation(orange_shader_program)) {
        std::abort();
        return 1;
    }
    ...
    if (!CheckProgramCompilation(yellow_shader_program)) {
        std::abort();
        return 1;
    }
